    ...
    return (
      <div className="card">
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="text-sm mb-4">Clique em "Nova Sessão" para inicializar uma conexão</p>
          <div className="flex items-center justify-center">
            <button
              onClick={() => {
                const input = window.prompt('Informe o número (apenas dígitos, ex: 5511999999999) para abrir o QR Code:');
                if (!input) return;
                const phone = input.replace(/\D/g, '');
                if (phone.length < 10) {
                  alert('Número inválido. Use apenas dígitos, incluindo DDI e DDD.');
                  return;
                }
                onReconnect(phone);
              }}
